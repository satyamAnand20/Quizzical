...
const LeaderboardPage = () => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      // Trigger confetti once manually if needed
      if (window.startConfetti) {
        window.startConfetti(); // Some libraries expose this
      }

      // Attempt to remove mobile event listeners that re-trigger it
      document.removeEventListener("touchstart", window.startConfetti, true);
      document.removeEventListener("scroll", window.startConfetti, true);
    };
